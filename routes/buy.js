...
exports.Usersearch = function(req, res){
	...
	var query = "SELECT * from userDetails where (firstname like "+ "'" + '%'+value+"') OR"+"(lastName like "+"'" +'%'+value+"') OR"+"(ssn like "+"'" +'%'+value+"')";
	...
	connection.query(query,function(err,rows){
		...
		else
		{
			if(rows.length>0)
				...
				res.render('user_list',{ title:
					'User details' ,rows:rows},function(err, result) {
				        // render on success
				        if (!err) {
				            res.end(result);
				        }
				...
				res.render('error',{ title:
					'No results found !! ' ,message: 'No Results found !! Please Try Again',back:'valid'});
				}
		}
...
exports.search = function(req, res){
			...
			if(rows.length>0)
				...
				res.render('product_list',{ title:
					'product details' ,rows:rows},function(err, result) {
				        ...
				        if (!err) {
				        	util.inspect(res);
					client.setex("search", 10,JSON.stringify(rows));
				            res.end(result);
				        }
				        // render or error
				        else {
	...
	dataPool.returnConnection(connection);
		...
		{
			util.inspect(reply);
			console.log("1: "+JSON.stringify(reply));
			res.render('product_list',{ title:
