...
function updateSelling(req,res,id)
{
	...
	var query = "update product set (productName,productType,category,quantity,brand,state,model,description) VALUES ('" + productName + "','"  + productType + "','"+ category +"','"+ quantity  + "','" +brand + "','"  + state + "','" + model +"','" + description  + "') where productId="+id;
	...
	var query = "update product  (price) set VALUES (" + price +") where productId="+id;
	console.log(" update product: "+query);
	connection.query(query,function(err,results){
		if (err) 
		{
